...
const StyledInterests = styled.div`
  display: grid;
  /* Calculate how many columns are needed, depending on technologies count */
...
const Technologies = ({ content }) => {
  ...
  const { shownItems, technologies } = exports
  ...
  useEffect(() => {
    // If mobile or tablet, show all technologies initially
    // Otherwise technologies.mdx will determine how many technologies are shown
    // (isSSR) is used to prevent error during gatsby build
    if (!isSSR && detectMobileAndTablet(window.innerWidth)) {
      setShownInterests(technologies.length)
    }
  }, [technologies])
  ...
  const showMoreItems = () => setShownInterests(shownInterests + 31)

  return (
    <StyledSection id="technologies">
      <StyledContentWrapper>
        <h3 className="section-title">{frontmatter.title}</h3>
        <StyledInterests itemCount={technologies.length} ref={ref}>
          {technologies.slice(0, shownInterests).map(({ name, icon }, key) => (
            <motion.div
              className="interest"
              key={key}
              custom={key}
          ...
          {shownInterests < technologies.length && (
